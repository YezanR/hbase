	...
	if(current_token == DROP)
	{
		...
		if(_table_name())
		{
			if(table_name_exists_in_TABSYM(cmd->table->name, &table_index))
			{
				...
				{
					printf("ERROR : table (%s) is enabled\n", cmd->table->name);
				}
			}
			else
			{
				if(DEBUG)
				{
					printf("ERROR table '%s' doesn't  exist in TABSYM\n", cmd->table->name);
				}
			...
			else
			{
				if(DEBUG)
				{
					printf("ERROR table '%s' doesn't  exist in database\n", cmd->table->name);
				}
			}
			result  = true;
		}
...
boolean _is_enabled()
{
	...
	if(current_token == IS_ENABLED)
	{
		...
		if(_table_name())
		{
			if(cmd->table->enabled == true && table_name_exists_in_TABSYM(cmd->table->name, &table_index))
			{
...
boolean _help(){
	...
	if(current_token== HELP)
	{
		...
		else if (current_token==DELETEALL){
			current_token=_read_token();
			if(current_token==APOST){
